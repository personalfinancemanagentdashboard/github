...
export interface IStorage {
  ...
  // Income Settings methods
  getIncomeSettings(userId: string): Promise<IncomeSettings | null>;
  upsertIncomeSettings(userId: string, monthlyIncome: string): Promise<IncomeSettings>;
}
...
export class DbStorage implements IStorage {
  ...
  // Income Settings methods
  async getIncomeSettings(userId: string): Promise<IncomeSettings | null> {
    const [settings] = await db.select().from(incomeSettings).where(eq(incomeSettings.userId, userId));
    return settings || null;
  }

  async upsertIncomeSettings(userId: string, monthlyIncome: string): Promise<IncomeSettings> {
    const [settings] = await db
      .insert(incomeSettings)
      .values({ userId, monthlyIncome })
      .onConflictDoUpdate({
        target: incomeSettings.userId,
        set: {
          monthlyIncome,
          updatedAt: new Date(),
        },
