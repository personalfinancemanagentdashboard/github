...
export const incomeSettings = pgTable("income_settings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
...
export const insertIncomeSettingsSchema = createInsertSchema(incomeSettings).omit({
  id: true,
  updatedAt: true,
});

export type InsertIncomeSettings = z.infer<typeof insertIncomeSettingsSchema>;
export type IncomeSettings = typeof incomeSettings.$inferSelect;
